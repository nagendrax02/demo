...
const canUpdateInternalSchemas = (): boolean => {
  const { updatedTo, selectedField } = getSelectedFieldValue();
  const internalSchemaList = ['OpportunityStatus', 'OpportunityReason'];
  const internalSchemaName = selectedField?.internalSchemaName;
  if (!internalSchemaName) return true;

  if (internalSchemaList?.includes(internalSchemaName) && !updatedTo?.value) {
    handleFocus(InputId.UpdateTo);
    return false;
  }
...
export const getOpportunityFields = (): IColumnField[] => {
...
const getOpportunityBody = (): IActivityBody | null => {
  ...
  return {
    ...
    ActivityFields: getOpportunityFields(),
...
export const handleOpportunitySave = async (
  onSuccess: (triggerRefresh?: boolean) => void
): Promise<void> => {
  const { initGridConfig } = getSelectedFieldValue();
  try {
    const body = getOpportunityBody();
...
export const handleOpportunityBulkUpdate = async (
  onSuccess: (triggerRefresh?: boolean) => void
): Promise<void> => {
  if (
    !canProceedWithUpdateTo([
    ...
    !canUpdateDropdown() ||
    !canUpdateInternalSchemas()
  ) {
    return;
  }

  await handleOpportunitySave(onSuccess);
};
