...
import { TabIconType, TabType } from '../app-tabs.types';
...
describe('Hook Utils', () => {
  ...
  describe('getActiveTabType', () => {
    it('Should return TabType.Secondary for secondary tabs', () => {
      // Act
      const result = getActiveTabType(APP_ROUTE.leadDetails);

      // Assert
      expect(result).toBe(TabType.Secondary);
    });

    it('Should return TabType.Primary for primary tabs', () => {
      // Act
      const result = getActiveTabType('testPage');

      // Assert
      expect(result).toBe(TabType.Primary);
    });
  });
