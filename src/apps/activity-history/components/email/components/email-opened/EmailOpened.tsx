...
import { getEmailOpenedText } from './utils';
import Subject from '../subject';
import AutomationText from '../automation-text';

export interface IEmailOpened {
  data: IAugmentedAHDetail;
}

// eslint-disable-next-line complexity
const EmailOpened = ({ data }: IEmailOpened): JSX.Element => {
  ...
  switch (campaignActivityType) {
    ...
    case EMAIL_OPENED.OPENED_NOTIFICATION_EMAIL:
      renderEmailOpened = (
        <>
          {getEmailOpenedText({ campaignActivityType, campaignActivityName, user, email })}{' '}
          {subject}
        </>
      );
      break;
    ...
    case EMAIL_OPENED.OPENED_AUTOMATION_EMAIL:
      ...
      break;
    default:
      return <div>Email Opened Activity</div>;
  }

  return <div data-testid="email-opened">{renderEmailOpened}</div>;
};
